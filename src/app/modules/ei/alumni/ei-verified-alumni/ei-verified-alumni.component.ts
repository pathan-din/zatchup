...
export interface verifiedAlumniListElement {
...
const ELEMENT_DATA: verifiedAlumniListElement[] = [
  {
    'SNo': 1, 
...
@Component({
  selector: 'app-ei-verified-alumni',
  templateUrl: './ei-verified-alumni.component.html',
  styleUrls: ['./ei-verified-alumni.component.css']
})
export class EiVerifiedAlumniComponent implements OnInit {
  model:any={};
  aluminiList:any=[];
  arrAge:any=[]
  pageSize:any=1;
...
/** Function name: displayCourseList
  ...
  getAluminiList(page,strFilter){
        ...
        this.SpinnerService.show(); 
        this.baseService.getData('ei/verified-alumni/',data).subscribe(res => {
        let response:any={};
        response=res;
        this.SpinnerService.hide(); 
...
getExportData(){
  try {
    ...
    this.baseService.generateExcel('ei/export-verifiedalumni-by-ei/', 'verified-alumni-list', params)
