...
import { StarclassCourseHistory } from '../ei/modals/education-institute.modal';

@Component({
  selector: 'app-admin-course-history',
  templateUrl: './admin-course-history.component.html',
  styleUrls: ['./admin-course-history.component.css']
})
export class AdminCourseHistoryComponent implements OnInit {
  ...
  getCourseHistory(page? : any){
    try {
      ...
      this.baseService.getData('starclass/course-history', this.starclassCourseHistory.modal).subscribe(
        (res: any) =>{
          if(res.status == true){
            if (!page)
